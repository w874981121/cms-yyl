 ...
 * 当前相对路径
 * */
const fike_url: string = path.join(__dirname, '../../../../upfile/img/');

export const ApiRes = async (ctx: any, next: any) => {
    // 上传单个文件
    const file = ctx.request.files.file; // 获取上传文件
    new UpFile(fike_url, file,(path:string)=>{
        console.log("写入成功")
        console.log(path)
    })
    ctx.body = {
...
class UpFile {
    ...
    constructor(path: string, file: any, callback: any) {
        this._path = path
        this._file = file
    ...
    upFile() {
        ...
        readStream.on('end',function(){
            ...
            writeStream.end('end',function(){
                ...
                // // 可读流通过管道写入可写流
                if (typeof that._callback === "function") return that._callback(filePath);
            });
        });
